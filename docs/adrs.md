...
**Decision.** v1 fronts exactly one downstream MCP server. A config naming more than one is rejected with an explicit "multi-server not supported in v1" error. The multi-server routing seam (per-server snapshot + policy) is documented for v2, not stubbed — that document is now ADR-J.
...
**SDK.** `@a2a-js/sdk@1.0.1` pinned EXACTLY, imported only under `src/a2a/`. Never `@latest` (0.3.x implements spec v0.3). The GA re-pin (from `1.0.0-beta.0`, 2026-07-28) cost zero source changes: the only breaking move was nine error classes relocating out of `./server` and `./client` into a new `./errors` subpath, and this lane catches none of them — deny paths publish terminal task states instead. ADR-I's relied-on server surface (`DefaultRequestHandler`, `InMemoryTaskStore`, `JsonRpcTransportHandler`, `AgentExecutor`) survived unrenamed, as did `./client`'s `ClientFactory`/`JsonRpcTransportFactory`/`ServiceParameters`. Wire spec stayed v1.0 (`A2A_PROTOCOL_VERSION` byte-identical across both versions), so the card scorer's W1 field classification needed no re-verification.
...
## ADR-I addendum 2 — the re-signing key (2026-07-05)

The last deferral is closed. `gatewarden a2a-keygen` mints an ES256/EdDSA card-signing pair (private JWK written 0600 + publishable jwks.json); `a2a-card --signing-key` emits a SIGNED card; `a2a-serve --signing-key` serves the signed card AND mounts `GET /.well-known/jwks.json`, with the protected header's `jku` pointing at the public interface origin's well-known JWKS. The signer (`signAgentCard`, vendored from mcp-fit) shares `canonicalCardPayload` with the verifier — one canonicalization, zero drift — and REPLACES any existing signatures (a stale upstream signature over a modified card is worse than none). Full-stack test: a signed card served over live HTTP verifies at `crypto-jku` against the live JWKS endpoint, and at `crypto-pinned` when the key store is supplied.

## ADR-J — Multi-downstream routing (the v2 seam, designed) — PROPOSED (design locked 2026-07-10; build gated on a design partner)

**Context.** ADR-D pinned v1 to one downstream and promised the multi-server seam "documented for v2". This is that document — produced by the 2026-07-10 strengthening session, which confirmed no other v2 design note existed. It answers the four open questions (routing shape, lease blast radius, A2A fan-out, snapshot economics). Nothing here is stubbed in code; R9's single-server rejection stays in force until a v2 build is committed.

**J1 — Routing/addressing: alias-prefixed tools on one aggregated face.** `downstreams` becomes a list with a REQUIRED unique `alias` per entry (zod-enforced uniqueness). `ToolActionMapping` gains a `downstream: alias` field; the resolver becomes `(alias, toolName, args) → Action`. The gateway serves ONE merged MCP face: `tools/list` unions downstream tools as `<alias>__<toolName>`; an inbound `tools/call` parses the prefix, routes to that downstream's client, and forwards the BARE name. Prefixing is uniform — mapped and passthrough tools alike (R5 semantics unchanged, applied per-downstream). Rejected: an explicit server-select parameter (no standard MCP field; `_meta` is invisible in `tools/list` and nonstandard) and one-endpoint-per-downstream (N gateways in one process — abandons the aggregation that makes a gateway a gateway). Precedent for the separator convention: MCP hosts already namespace as `mcp__server__tool`; Kong AI Gateway and agentgateway both rewrite tool listings — clients tolerate it. Session/lease binding mechanics are unchanged (token bound at `initialize` via `_meta['x-lease-token']`).

**J2 — Lease scope across servers: per-capability server binding, deny-by-default.** A lease's scopes are meaningful only RELATIVE to a downstream — `./data/**` on two servers is two different filesystems, so a lease silently spanning all fronted servers is a blast-radius bug, not a convenience. Decision: capabilities gain an optional `servers?: string[]` (alias list) in the Lease claims; `Enforcer.check` gains one step — `action.downstream ∈ capability.servers` — before scope matching. Multi-downstream mode + absent field → DENY (deny-by-default). Single-downstream mode + absent field → the sole server (full back-compat; existing tokens keep working). A lease spans servers only by explicitly naming each alias — never implicitly. Cross-repo dependency: this is a leasebroker lease-schema MINOR (additive claim; the existing unknown-key tolerance holds) plus one Enforcer step — it belongs in govern's canonical contract and arrives here via the ADR-0 vendoring path. It mirrors the A2A lane's endpoint-allow-list "which counterparty" dimension (ADR-H): same mental model, reuses the scope-check machinery, no new capability kind.

**J3 — A2A fan-out: static config-declared set; dynamic routing REJECTED for v2.** v2 fronts N KNOWN remote agents (N `A2aGatewaySnapshot`s, J1 aliases). Dynamic routing to caller-named agents is a materially different product (an A2A hub/router) and is rejected explicitly so the implementation cannot back into it: request-time card fetch+score breaks ADR-B's score-at-attach trust model (attach IS the trust decision); caller-supplied agent URLs are an SSRF-shaped surface with the endpoint allow-list as the only guard; and discovery/unknown-agent trust is its own product decision. Revisit as v3+ on explicit demand. (Platform precedent is also static-configured: AWS AgentCore Gateway.)

**J4 — Snapshot economics: per-alias, fail-closed, never blended.** Snapshots become `Map<alias, GatewaySnapshot>`; attach is O(N) keyless deterministic lint (no economics problem); `rescore <alias>` is selective; per-snapshot immutability is unchanged (ADR-B holds). Partial attach FAILS CLOSED by default (all-or-nothing); an explicit opt-in degraded mode may serve the attached subset with unattached aliases' tools ABSENT from `tools/list` (absent, not denied — never advertise what cannot be served). Per-alias scorecards only — blending scores into one gateway aggregate would launder a bad server behind a good one's number. Entrained changes: audit events gain a `downstream: alias` field; R9 flips to alias-uniqueness validation; the CLI grows per-alias `score`/`rescore`; the generated A2A card (ADR-H) prefixes skill ids by alias per the J1 convention.

**Build gate.** Design locked; build deferred until a design partner has a real multi-server need (2026-07-10 PMF read: the toll booth needs traffic before lanes). Exception: J2's `servers[]` binding may be pulled forward independently — it strengthens the lease format's standardization story regardless of when v2 ships.

**Sentiment corroboration (2026-07-11, banked for design-partner conversations, not build licence).** The Levie enterprise-adoption thread's single biggest cluster — Operating Model & Ownership, 34/115 comments ("who manages agents that cross organizational boundaries") — independently validates ADR-J's direction. The build gate above is unchanged.
