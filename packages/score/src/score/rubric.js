 ...
 */
export async function generateRubric(client, model, task, toolNames, maxTokens) {
...
Higher weight = more important for this task.`;
    ...
    try {
        ...
        const criteria = parsed.criteria.map((c, i) => ({
 ...
 */
export async function scoreTrace(client, model, rubric, trace, round, maxTokens) {
 ...
 */
export async function runRubricLoop(task, trace, toolNames, options = {}) {
