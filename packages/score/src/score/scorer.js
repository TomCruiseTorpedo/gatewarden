 ...
 */
function rubricToAxisScores(results) {
    if (results.length === 0) {
        const perAxisMean = {};
    ...
    const overallVariance = allScores.reduce((sum, s) => sum + (s - overallMean) ** 2, 0) / allScores.length;
 ...
 */
function buildAxisScores(lintResult, rubricResults) {
    ...
    for (const axis of AXIS_NAMES) {
        ...
        const lintFindings = lintAxisScore.findings;
        if (!hasEval) {
...
// ---------------------------------------------------------------------------
function buildAggregate(lintResult, rubricResults) {
    ...
    const combined = Math.round(((lintScore + evalWeighted) / 2) * 10) / 10;
    return {
        lintScore,
        evalScore,
        weighted: combined,
 ...
 */
export function scoreLintOnly(server, lintResult) {
