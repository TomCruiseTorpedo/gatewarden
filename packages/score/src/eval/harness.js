 ...
 */
export function classifyProvenance(value, taskDescription, priorReturns) {
 ...
 */
export function analyzeToolCallProvenance(toolName, args, taskDescription, priorReturns) {
 ...
 */
export function computePreliminaryRubric(task, chosenTools) {
...
// ---------------------------------------------------------------------------
function inferPass(task, chosenTools) {
    if (!task.expectedTools || task.expectedTools.length === 0) {
...
export class ClaudeHarness {
     ...
     */
    async runTask(task, toolset, sandbox) {
        ...
        for (let turn = 0; turn < this.maxTurns; turn++) {
            ...
            if (response.stop_reason === 'tool_use') {
                ...
                for (const tu of toolUseBlocks) {
                    ...
                    const args = tu.input != null && typeof tu.input === 'object'
                        ? tu.input
                        : {};
                    // Analyse provenance before adding this tool's result to priorReturns
                    const events = analyzeToolCallProvenance(tu.name, args, task.description, priorReturns);
