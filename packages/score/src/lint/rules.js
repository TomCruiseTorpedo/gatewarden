...
function levenshtein(a, b) {
    ...
    const row1 = new Array(n + 1).fill(0);
    for (let i = 1; i <= m; i++) {
        row1[0] = i;
        for (let j = 1; j <= n; j++) {
            row1[j] =
 ...
 */
const noMissingToolDescription = {
    id: 'no-missing-tool-description',
    axis: 'namespacing',
    description: 'Every tool must have a non-empty description so agents can identify it.',
    check(tool) {
...
const vagueToolName = {
    id: 'vague-tool-name',
    axis: 'namespacing',
    description: 'Tool names that are too short or domain-generic confuse agent tool selection.',
    check(tool) {
...
const prefixConsistency = {
    ...
    axis: 'namespacing',
    description: 'Tools in the same server should use a consistent naming prefix to aid discovery.',
    check(tool, allTools) {
        ...
        const prefixes = allTools.map(t => getPrefix(t.name)).filter((p) => p !== null);
 ...
 */
const overlappingToolNames = {
    id: 'overlapping-tool-names',
    axis: 'tool-selection-confusion',
    description: 'Tools with very similar names may mislead agent tool selection.',
    check(tool, allTools) {
        const findings = [];
 ...
 */
const noRequiredParamDescription = {
    id: 'no-required-param-description',
    axis: 'param-strictness',
    description: 'Every required parameter must have a non-empty description for unambiguous call signatures.',
    check(tool) {
        const findings = [];
...
const noParamType = {
    id: 'no-param-type',
    axis: 'param-strictness',
    description: 'Every parameter should declare a JSON Schema type for strict call signatures.',
    check(tool) {
        const findings = [];
 ...
 */
const optionalRequiredConfusion = {
    id: 'optional-required-confusion',
    axis: 'param-strictness',
    description: 'Parameters named "optional" must not appear in the required list.',
    check(tool) {
        const findings = [];
        const { properties = {}, required = [] } = tool.inputSchema;
...
const proseOutputHint = {
    id: 'prose-output-hint',
    axis: 'output-leanness',
    description: 'Descriptions hinting at prose/unstructured output predict token bloat.',
    check(tool) {
        if (!tool.description)
            return [];
                ...
                return [
                    {
                        ...
                        severity: 'warning',
                        tool: tool.name,
                        message: `Tool "${tool.name}" description hints at prose output` +
 ...
 */
const noOutputSchema = {
    id: 'no-output-schema',
    axis: 'output-leanness',
    description: 'Tools should declare outputSchema to communicate structured output contracts.',
    check(tool) {
 ...
 */
const ERROR_SIGNAL_RE = /\b(error|fail|exception|throw|invalid|not\s+found|does\s+not\s+exist|returns?\s+null|undefined)\b/i;
const noErrorDocs = {
    id: 'no-error-docs',
    axis: 'error-helpfulness',
    description: 'Tool descriptions should document error cases so agents can plan recovery.',
    check(tool) {
        if (!tool.description)
            return []; // no-missing-tool-description covers this
