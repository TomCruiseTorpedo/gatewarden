...
export class InMemoryAuditSink {
    ...
    append(event) {
        ...
        const withPrevHash = { ...event, prevHash };
    ...
    read() {
        for (let i = 0; i < this.events.length; i++) {
            ...
            if (event.prevHash !== expectedPrevHash) {
                throw new Error(`Audit log tampered at event[${i}]: prevHash mismatch ` +
            ...
            if (event.hash !== expectedHash) {
                throw new Error(`Audit log tampered at event[${i}]: hash mismatch ` +
