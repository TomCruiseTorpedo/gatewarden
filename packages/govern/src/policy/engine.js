    ...
    #scopeIsAllowed(rule, cap) {
        switch (cap.kind) {
            ...
            case 'fs.write': {
                ...
                return cap.paths.every(reqPath => rule.paths.some(rulePath => pathIsCovered(reqPath, rulePath)));
            }
            case 'http.call': {
                if (rule.endpoints === undefined || rule.endpoints.length === 0)
                    return true;
                return cap.endpoints.every(reqEndpoint => rule.endpoints.some(ruleEndpoint => pathIsCovered(reqEndpoint, ruleEndpoint)));
            }
            case 'spend': {
 ...
 */
function pathIsCovered(requested, rulePattern) {
    if (rulePattern === requested)
        return true;
