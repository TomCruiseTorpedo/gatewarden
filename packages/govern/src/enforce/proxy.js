...
import { CallToolRequestSchema, InitializeRequestSchema, LATEST_PROTOCOL_VERSION, ListToolsRequestSchema, SUPPORTED_PROTOCOL_VERSIONS, } from '@modelcontextprotocol/sdk/types.js';
// ---------------------------------------------------------------------------
// LeasebrokerProxy
// ---------------------------------------------------------------------------
export class LeasebrokerProxy {
    ...
    constructor(opts) {
        this.opts = opts;
        this.server = new Server({ name: 'leasebroker-proxy', version: '1.0.0' }, { capabilities: { tools: {} } });
        this.downstreamClient = new Client({ name: 'leasebroker-proxy-downstream', version: '1.0.0' }, { capabilities: {} });
    ...
    installHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
            ...
            const token = sessionId !== undefined ? this.sessionTokens.get(sessionId) : undefined;
            if (token === undefined) {
     ...
     */
    async connect(clientTransport, downstreamTransport) {
