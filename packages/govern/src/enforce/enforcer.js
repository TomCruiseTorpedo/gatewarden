...
export class LeaseEnforcer {
     ...
     */
    registeredCaps = new Set();
    constructor(signer, revocationList, spendLedger) {
        this.signer = signer;
 ...
 */
function checkScope(lease, action) {
    for (const cap of lease.capabilities) {
        if (cap.kind !== action.kind)
            continue;
        if (action.kind === 'fs.read' && cap.kind === 'fs.read') {
            if (cap.paths.some((p) => minimatch(action.path, p)))
                return { ok: true };
        }
            ...
            if (cap.endpoints.some((e) => minimatch(action.endpoint, e)))
                return { ok: true };
        }
        else if (action.kind === 'spend' && cap.kind === 'spend') {
            if (cap.currency === action.currency)
                return { ok: true };
        }
    }
    return {
