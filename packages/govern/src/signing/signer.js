 ...
 */
export class PasetoV4PublicSigner {
     ...
     */
    constructor(signingKey, additionalKeys) {
    ...
    verify(token) {
        ...
        try {
            ...
            const claims = JSON.parse(Buffer.from(rawMessage).toString('utf-8'));
        ...
        // ── 5. Parse lease ────────────────────────────────────────────────────────
        try {
            const lease = JSON.parse(Buffer.from(result.payload).toString('utf-8'));
            return { lease };
        }
        catch {
