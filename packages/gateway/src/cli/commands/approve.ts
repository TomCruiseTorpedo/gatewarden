/**
 * `gatewarden approve <reqId>` — approve a pending (veto-required) request.
 *
 * Retrieves the pending request, issues a lease, and prints the granted token.
 *
 * Output (JSON):
...
export function cmdApprove(state: CliState, opts: ApproveOptions): void {
  ...
  if (outcome.type === 'granted') {
    console.log(
      JSON.stringify({ type: 'granted', token: outcome.token, leaseId: outcome.lease.id }),
    );
  } else {
